(function () {
  ...
  schemaForm.provider('$validator', [
    '$filterProvider',
    function $validator($filterProvider) {
      this._validators = {};
      this._formatters = {};
    ...
    function schemaFormDirective($injector, Restangular, $cacheFactory) {
      return {
        ...
        link: function postLink(scope) {
          var model = $injector.get(scope.schemaName), schema = model.getSchema(), metadata = model.getMetadata(), orderedSchema = [], Ref, resource, refWatches = {}, objectCache = $cacheFactory.get('objects'), plural, cachedData;
          scope.refData = {};
          scope.refSchema = {};
          scope.refMetadata = {};
          angular.forEach(schema, function (def, field) {
            if (angular.isString(def)) {
              schema[field] = { type: def };
            }
            ...
            if (angular.isDefined(metadata.placeholders) && metadata.placeholders[field]) {
  ...
  schemaForm.directive('validate', [
    '$validator',
    function validateDirective($validator) {
      return {
        restrict: 'A',
        require: 'ngModel',
          ...
          if (type) {
            ngModel.$parsers.unshift(function (viewValue) {
              ...
              if (viewValue || attrs.required) {
                valid = $validator.validate(type, $validator.format(type, viewValue));
              }
